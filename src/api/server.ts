	...
	await fastify.register(swagger, {
		openapi: {
			...
			info: {
				...
				version: '1.0.0',
				description:
					'دليل استخدام واجهة برمجة تطبيقات واتساب. يمكنك إرسال الرسائل من خلال الـ API، واستقبال التحديثات عبر الـ Webhook.'
			},
			components: {
				securitySchemes: {
			...
			webhooks: {
				WhatsAppEvents: {
					post: {
						...
						description:
							'نقطة النهاية (Endpoint) الخاصة بك يجب أن تكون جاهزة لاستقبال طلبات `POST` تحتوي على هذا الـ JSON.',
						requestBody: {
							content: {
								'application/json': {
									schema: {
...
* \`device_logged_out\`: تنبيه أمني وحرج يتم إرساله إذا قام المستخدم بتسجيل الخروج من تطبيق واتساب في هاتفه.`
	...
	createBullBoard({
		queues: [new BullMQAdapter(messageQueue), new BullMQAdapter(webhookQueue)],
		serverAdapter
	})
	...
	fastify.put(
		'/instances/:id/token',
		{
			schema: {
				...
				description:
					'تسمح لك هذه النقطة بتحديث التوكين السري الخاص بالجهاز. ملاحظة: يجب إرسال التوكين الحالي الصالح في ترويسة (Authorization Bearer) لتتمكن من تغييره.',
				tags: ['Settings'],
				security: [{ bearerAuth: [] }],
				params: {
					type: 'object',
	...
	fastify.post(
		'/instances/:id/messages/send-json',
		{
			schema: {
